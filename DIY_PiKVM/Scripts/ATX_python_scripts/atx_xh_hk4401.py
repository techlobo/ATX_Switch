...
#    ATX Switch - KVM driver modified to enable integration with ATX switch   #
#                                                                             #
#    Copyright (C) 2018-2024  Maxim Devaev <mdevaev@gmail.com>                #
#                  2021-2021  Sebastian Goscik <sebastian.goscik@live.co.uk>  #
#                  2023-2024  techlobo <techlobo@gmail.com>                   #
#                                                                             #
...
import os
import sys
sys.path.append("/etc/atx_switch/")
...
from change_atx_from_kvm import *
...
class Plugin(BaseUserGpioDriver):  # pylint: disable=too-many-instance-attributes
    ...
    def get_pin_validator(cls) -> Callable[[Any], Any]:
        return functools.partial(valid_number, min=0, max=3, name="ATX_XH-HK4401 channel")
    ...
    def __serial_worker(self) -> None:
        logger = aioproc.settle(str(self), f"gpio-atx_xh-hk4401-{self._instance_name}")
...
# Check if system just been initialised, and if so set ATX port to match the selected KVM channel
        if os.path.exists('/var/tmp/atx_switch/atx_kvm_first_time'):
            if channel != None:
                os.remove('/var/tmp/atx_switch/atx_kvm_first_time')
                logger.info(f'Setting initial ATX port to match KVM channel...{channel + 1}')
                change_atx_from_kvm(channel + 1)
...
# Set ATX selected port to match KVM channel number
        change_atx_from_kvm(channel + 1)

    def __str__(self) -> str:
        return f"ATX_XH-HK4401({self._instance_name})"
